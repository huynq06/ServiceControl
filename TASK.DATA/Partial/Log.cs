...
namespace TASK.DATA
{
    public partial class Log
    {
        ...
        public static void WriteSeriviceLog(string logText, string fileName)
        {
            try
            {
                logText = string.Format("========={0}=========\n{1}\n", DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"), logText);
                var path = @"C:\LogService\";
        ...
        public static void WriteCfsLog(string content,string fileName)
        {
            try
            {
              //  logText = string.Format("========={0}=========\n{1}\n", DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"), logText);
                ...
                lock (_locker)
                {
                    using (Stream s = new FileStream(fullPath, FileMode.Create, FileAccess.Write, FileShare.ReadWrite))
                    {
                        using (StreamWriter w = new StreamWriter(s))
                        {
                            w.WriteLine(content);
